...
export default function NewWallet() {
  ...
  const handleContinue = async () => {
    if (currentStep === 1) {
      ...
      if (confirmedPhrase.some((word) => word === "")) {
        alert("Please fill in all the words in the phrase.");
        return;
      }

      // create wallet and store in local storage
      const { publicKey, privateKey } = generateWallet(phrase.join(" "));
      ...
      const hashedSeedPhrase = hashSeedPhrase(confirmedPhrase.join(" "));
